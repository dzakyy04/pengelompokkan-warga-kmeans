...
<div class="max-w-lg">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mt-4">
        <form method="POST" action="{{ route('admin.master-aset.update', $item) }}">@csrf @method('PUT')
            ...
            <input type="number" name="estimasi_nilai" value="{{ old('estimasi_nilai', $item->estimasi_nilai) }}" required min="0" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500 outline-none">@error('estimasi_nilai')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror</div>
