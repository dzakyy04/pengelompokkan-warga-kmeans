...
@section('page-title', 'Data Pendidikan')
@section('content')
<div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
    <div><h1 class="text-2xl font-bold text-gray-900 dark:text-white">Data Pendidikan Kepala Keluarga</h1><p class="text-gray-500 dark:text-gray-400 text-sm mt-1">Kelola data master tingkat pendidikan</p></div>
    <button onclick="openModal('create')" class="w-full md:w-auto justify-center inline-flex items-center px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold rounded-xl transition shadow-sm">
        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>Tambah Pendidikan
    </button>
</div>
<div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden transition-colors duration-200">
    <table class="w-full text-sm">
        <thead class="bg-gray-50 dark:bg-slate-700/50 border-b border-gray-200 dark:border-slate-700"><tr>
            ...
            <th class="px-4 py-3 text-left font-semibold text-gray-600 dark:text-gray-300 text-xs uppercase">Tingkat Pendidikan</th>
        ...
        <tbody class="divide-y divide-gray-200 dark:divide-slate-700">
            ...
            <tr class="hover:bg-emerald-50/50 dark:hover:bg-slate-700/50 transition-colors">
                ...
                <td class="px-4 py-3 text-center"><span class="px-2 py-1 rounded-lg text-xs font-semibold {{ match(true) { $item->skor >= 5 => 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400', $item->skor >= 3 => 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400', default => 'bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400' } }}">{{ $item->skor }}</span></td>
                ...
                <td class="px-4 py-3 text-center"><div class="flex items-center justify-center gap-1">
                    ...
                    <form method="POST" action="{{ route('admin.master-pendidikan.destroy', $item) }}" onsubmit="return confirm('Hapus?')">@csrf @method('DELETE')<button class="p-1.5 text-gray-400 dark:text-gray-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button></form>
                </div></td>
            </tr>
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="relative bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl shadow-2xl w-full max-w-md p-6 z-10 transform transition-all">
            <div class="flex items-center justify-between mb-4">
                <h3 id="modalTitle" class="text-lg font-bold text-gray-900 dark:text-white">Tambah Pendidikan</h3>
            ...
            <form id="modalForm" method="POST">
                ...
                <div class="mb-4">
                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Nama Tingkat Pendidikan <span class="text-red-500">*</span></label>
                    <input type="text" name="nama" id="inputNama" required class="w-full px-4 py-2.5 border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-white rounded-xl text-sm focus:ring-2 focus:ring-emerald-500 outline-none transition-colors">
                    @error('nama')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Skor (1-10) <span class="text-red-500">*</span></label>
                    <input type="number" name="skor" id="inputSkor" min="1" max="10" required class="w-full px-4 py-2.5 border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-white rounded-xl text-sm focus:ring-2 focus:ring-emerald-500 outline-none transition-colors">
                    <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">Semakin tinggi nilai, semakin tinggi tingkat pendidikan.</p>
...
function openModal(mode, id, nama, skor) {
    ...
    if (mode === 'edit') {
        ...
        form.action = '{{ url("admin/master-pendidikan") }}/' + id;
        ...
        form.action = '{{ route("admin.master-pendidikan.store") }}';
        method.innerHTML = '';
